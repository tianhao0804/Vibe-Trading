...
This map is the FROZEN canonical Robinhood catalog (SPEC §7.5) for the current
Agentic MCP surface. Any tool the broker reports that is not in this map and not
annotated read-only resolves to UNKNOWN → treated as WRITE (fail-closed), so an
unrecognized new broker tool can never slip through as a plain read. Adding a
tool here is a localized edit to this one dict plus the classification test
parametrize list.
"""
