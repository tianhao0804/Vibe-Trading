...
def test_extractor_maps_notional_order() -> None:
    intent = extract_order_intent(
        "place_equity_order",
        {"symbol": "aapl", "side": "buy", "dollar_amount": 250.0, "type": "market"},
    )
...
def test_extractor_maps_quantity_and_dollar_amount_alias() -> None:
    intent = extract_order_intent(
        "place_equity_order",
        {"ticker": "NVDA", "action": "sell", "quantity": 3, "dollar_amount": 600, "type": "market"},
    )
    assert intent is not None
...
def test_extractor_ignores_unknown_extra_keys() -> None:
    intent = extract_order_intent(
        "place_equity_order",
        {
            "symbol": "MSFT", "side": "buy", "quantity": 1, "type": "limit",
            "time_in_force": "gtc", "client_tag": "x", "extended_hours": True,
        },
    )
    assert intent is not None
...
def test_extractor_rejects_missing_or_ambiguous_fields() -> None:
    # Missing side.
    assert extract_order_intent("place_equity_order", {"symbol": "AAPL", "dollar_amount": 10}) is None
    # Unknown instrument stays rejected on the legacy generic tool.
    assert extract_order_intent("place_order", {"symbol": "AAPL", "side": "buy", "instrument_type": "warrant", "notional_usd": 10}) is None
    # No size at all.
    assert extract_order_intent("place_equity_order", {"symbol": "AAPL", "side": "buy"}) is None
...
class _BrokerQuoteAdapter:
    ...
    def call_tool(self, remote_name: str, arguments: dict, *, local_name: str | None = None) -> dict:
        ...
        if remote_name in ("get_equity_quotes", "get_quotes"):
            ...
            return {"status": "ok", "results": [{"symbol": symbols[0], "last_price": self._price}]}
